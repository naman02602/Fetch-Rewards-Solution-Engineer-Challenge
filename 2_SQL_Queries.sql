...
WITH current_month AS (
    ...
    JOIN ReceiptItems ri ON r.receipt_id = ri.receipt_id
	JOIN Item_Brand ib ON ri.barcode = ib.barcode
    WHERE r.purchase_date > EOMONTH(GETDATE(), -1) 
),
...
previous_month AS (
    ...
    JOIN ReceiptItems ri ON r.receipt_id = ri.receipt_id
	JOIN Item_Brand ib ON ri.barcode = ib.barcode
    WHERE r.purchase_date >= DATEADD(DAY, 1, EOMONTH(GETDATE(), -2)) 
          AND r.purchase_date <= EOMONTH(GETDATE(), -1)
),
...
SELECT 
    TOP 5 COALESCE(cmr.brand_name, pmr.brand_name) AS brand_name, 
...
WITH recent_users AS (
    SELECT user_id
    FROM Users
    WHERE created_date >= DATEADD(DAY, 1, EOMONTH(GETDATE(), -6))  -- Users created in the last 6 months
),
...
WITH recent_users AS (
    SELECT user_id
    FROM Users
    WHERE created_date >= DATEADD(DAY, 1, EOMONTH(GETDATE(), -6))  -- Get users created in the last 6 months
),
